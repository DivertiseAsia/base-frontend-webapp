...
let getResponseMsgFromJson = json => {
  ...
  let splitMsg =
    Js.String.split(":", Js.String.replaceByRe(re, "", jsonString));
...
let floatWithoutZero = (number:string) => {
  let splitNumber = Js.String.split(".", number);
  let decimalNumber = splitNumber[1] |> int_of_string;

  splitNumber[0] ++ 
    (((decimalNumber mod 10) === 0 && decimalNumber !== 0) ? 
        ("." ++ Js.String.charAt(0, splitNumber[1])) 
          : (decimalNumber === 0 ? "" :
            "." ++ splitNumber[1]));
};

let checkWholeNumberFromStr = (text) =>
  switch (Js.String.match(Js.Re.fromStringWithFlags("^\\s*\\d+\\s*$", ~flags="g"), text)) {
...
let wholeNumberFromString = (text:string, ~fallback:int):int => {
  switch (checkWholeNumberFromStr({j|$text|j})) {
    | true => try (int_of_string({j|$text|j})) {
      | _ => fallback
    }
    | false => fallback
  }
...
let checkFloatfromStr: (string) => bool = [%bs.raw
{|
  function(value) {
...
let checkPasswordValidation: (string) => bool = [%bs.raw
{|
  function(value) {
...
module MapOption = {
  ...
  let mapOptFloatToStr = optFloat =>
    optFloat |> mapOptFloat |> Js.Float.toString;
};

let checkStr = (~text, ~limit) =>
  switch (
    Js.String.match(Js.Re.fromStringWithFlags(".*\\S.*", ~flags="g"), text)
  ) {
  | None => ""
  | Some(arr) =>
    Js.String.slice(~from=0, ~to_=limit, Js.Array.joinWith("", arr))
  };
...
let checkNumber = (~text, ~limit) =>
  switch (
    Js.String.match(Js.Re.fromStringWithFlags("\\d", ~flags="g"), text)
  ) {
  | None => ""
  | Some(arr) =>
    Js.String.slice(~from=0, ~to_=limit, Js.Array.joinWith("", arr))
...
let isStringEmpty = str =>
  str |> Js.String.length === 0 || str === "" || str === "-";
...
let searchCaseInsensitive = (searchKeyword, fullText) => {
  let onlyWord =
    matchOptionarrStrToStr(
      Js.String.match(
        Js.Re.fromStringWithFlags("[\\w\\s*]+", ~flags="g"),
        searchKeyword,
      ),
    );
  ...
  matchOptionarrStrToStr(
    Js.String.match(
      Js.Re.fromStringWithFlags(searchKeyLower, ~flags="g"),
      fullTextLower,
    ),
  );
};
...
let getSearchParameter = (parameter:string, queryString:string):option(string) => {
  let regex = Js.Re.fromStringWithFlags({j|$parameter=([^&#]*)|j}, ~flags="g");
  let rawValue = Js.String.match(regex, queryString);
  switch (rawValue) {
    | None => None
    | Some(arr) =>
      switch (Js.String.replace({j|$parameter=|j}, "", Js.Array.joinWith("", arr))) {
      | "" => None
      | x => Some(x)
      }
    };
