      ...
      <div className="container"> 
        <div className="row mt-3"> 
          <div className="col-2 text-center"> 
            <input
              disabled=(state.sending || state.hasSuccess)
              onChange={e => dispatch(SetField("checkbox", Utils.boolFromCheckbox(e) |> string_of_bool))}
              type_="checkbox"
              required=true
            />
          </div>
          <div className="col-10"> 
            <p>{string("Lorem lpsum is simple dummy text of the printing and typesetting industry.")}</p>
          </div>
        </div>
      </div>
      <input type_="submit"
        className="btn btn-filled btn-filled-auth-page btn-default mb-4" value="Next" 
      />
    </form>
    {state.sending === true ? <Loading /> : React.null}
    {
      state.hasSuccess ?
        <div className="text-info"> {string("Registration complete. You will need to be approved to login.")} </div> : null
    }
    {
